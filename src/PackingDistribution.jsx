...
function PackingDistribution({ orderId, onBack, showConfirmButton = false, onConfirmBoxes }) {
  ...
  const handleConfirmBoxes = async () => {
    if (!distribution) return
    if (!window.confirm('¿Confirmar todas las cajas de esta orden? La orden pasará a estado READY.')) return

    const lines = []
    for (const caja of distribution.cajas) {
      for (const producto of caja.productos) {
        lines.push({
          sku: producto.sku,
          quantity_served: producto.cantidad_en_caja,
          box_code: caja.codigo_caja,
        })
      }
    }
    ...
    try {
      const result = await orderService.batchUpdate(distribution.order.numero_orden, lines)
      if (result.status === 'error') {
        setError(result.message)
      } else {
        setSuccessMessage(`Orden ${distribution.order.numero_orden} confirmada exitosamente`)
        if (onConfirmBoxes) {
          setTimeout(() => onConfirmBoxes(result), 1500)
        } else if (onBack) {
